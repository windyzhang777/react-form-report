...
const CommonDataGrid = (props: CompDataGrid) => {
    const { reportStatus, reportIndex } = props;
    ...
    const highlightDate = (rowApi: any) => {
        ...
        let current = moment().add(-3, 'days').format('MM/DD/YYYY h:mm');
    ...
    { id: 4, logpagenumber: '67563432', reportedby: 'User 3', reportedid: 'U187654', datetime: moment().add(-2, 'days').format(dateFormat), logpagestatus: 'Open', sdrreportableitem: 'Yes' },
    { id: 5, logpagenumber: '87654342', reportedby: 'User 4', reportedid: 'U23423', datetime: moment().format(dateFormat), logpagestatus: 'Open', sdrreportableitem: 'No' },
    { id: 6, logpagenumber: '98765432', reportedby: 'User 5', reportedid: 'U685322', datetime: moment().add(-1, 'days').format(dateFormat), logpagestatus: 'Open', sdrreportableitem: 'Yes' },
    { id: 7, logpagenumber: '876541134', reportedby: 'User 6', reportedid: 'U90433', datetime: moment().add(-3, 'days').format(dateFormat), logpagestatus: 'Open', sdrreportableitem: 'No' },
    { id: 8, logpagenumber: '087654322', reportedby: 'User 7', reportedid: 'U56789', datetime: moment().add(-1, 'days').format(dateFormat), logpagestatus: 'Open', sdrreportableitem: 'No' },
    { id: 9, logpagenumber: '876543212', reportedby: 'User 8', reportedid: 'U32146', datetime: moment().format(dateFormat), logpagestatus: 'Open', sdrreportableitem: 'Yes' },
    ];
    ...
    useEffect(() => {
        // setRowData(sdrData?.filter((r) => r.logpagestatus === reportStatus));
        setRowData(sdrData);
        props.updateOpenSdrCount(sdrData.length);
    }, [reportStatus]);

    const onRowsSelectionHandler = (ids: any) => {
        const selectedRowsData = ids.map((id: string) => rowData?.find((row) => row.id.toString() === id.toString()));
